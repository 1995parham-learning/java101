...
class AppTest {
  ...
  @Test
  void parhamHasManyFriends() {
    Person parham = new Person("Parham", "Alvani");

    parham.addFriends(
        new Person("Elahe", "Dastan"),
        new Person("Raha", "Dastan"));

    parham.getFriends().forEach(item -> {
      assertEquals("Dastan", item.getLastName());
    });
  }
}
